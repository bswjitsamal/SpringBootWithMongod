...
import com.programming.techie.expensetracker.controller.ExpenseController;
...
public class ExpenseControllerTest {
	...
	void shouldCreateExpense() throws Exception {
		ExpenseDto expenseDto = ExpenseDto.builder().id("123456")
		...
		MvcResult mvcResult = mockMvc.perform(post("/api/expense").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(expenseDto))).andDo(print())
				.andExpect(MockMvcResultMatchers.status().isCreated())
				.andExpect(MockMvcResultMatchers.header().exists(HttpHeaders.LOCATION)).andReturn();
		assertTrue(Objects.requireNonNull(mvcResult.getResponse().getHeaderValue(HttpHeaders.LOCATION)).toString()
				.contains("123"));
	}
	...
	void shouldReturn404ErrorResponseForGETWithInvalidId() throws Exception {
		Mockito.when(expenseService.getExpense("123"))
				.thenThrow(new ExpenseNotFoundException("Cannot find Expense By id - 123"));
		MvcResult mvcResult = mockMvc.perform(get("/api/expense/123").contentType(MediaType.APPLICATION_JSON_VALUE))
